...
def components_csv_from_list_of_code_list_urls(code_list_urls_from_dataset, details):
    ...
    for measure in details["attributes to add"]["Measure Type"].keys():
        ...
        data["Path"].append("measure/cmd/{}".format(pathify(measure)))
        data["Range"].append("")
        data["Parent"].append("")
    ...
    for attribute in attributes_to_add:
        ...
        data["Codelist"].append("")
        data["Path"].append("measure/cmd/{}".format(pathify(attribute)))
        data["Range"].append("")
        data["Parent"].append("")
    for code_list_url in code_list_urls_from_dataset:
        # Get the codelist id
        ...
        data["Path"].append("measure/cmd/{}".format(pathify(code_list_id)))
        data["Range"].append("")
        data["Parent"].append("")
...
def generate_reference_data():
    ...
    # Make sure we have a referece directory with a codelists folder
    outputFolder = Path('reference/codelists')
